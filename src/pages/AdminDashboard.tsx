...
export function AdminDashboard() {
  ...
  return (
    <AdminShell>
      <header className="admin-header"><div><span className="page-kicker">VISÃO GERAL</span><h1>Perfis recebidos</h1><p>Acompanhe as inscrições e organize os próximos passos.</p></div><div className="admin-header__date"><span>Hoje</span><strong>{new Intl.DateTimeFormat('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' }).format(new Date())}</strong></div></header>

      <section className="stat-grid">
        <article><div className="stat-icon stat-icon--lime"><UsersRound /></div><span><small>Perfis recebidos</small><strong>{total}</strong></span></article>
        <article><div className="stat-icon stat-icon--purple"><Sparkles /></div><span><small>Em análise</small><strong>{countFor('Nova inscrição') + countFor('Analisando')}</strong></span></article>
        <article><div className="stat-icon stat-icon--blue"><MessageCircleMore /></div><span><small>Conversas</small><strong>{countFor('Quero conversar') + countFor('Conversa realizada')}</strong></span></article>
        <article><div className="stat-icon stat-icon--orange"><UserRoundCheck /></div><span><small>Selecionados</small><strong>{countFor('Selecionado')}</strong></span></article>
      </section>
